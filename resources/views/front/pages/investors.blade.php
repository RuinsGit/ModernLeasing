...
@section('content')
        ...
        <div class="container">
            <div class="row">
                <div class="col-12 text-center mb-5">
                    @if(isset($partnershipSection) && $partnershipSection->title)
                        <h2 class="section-title text-white" data-aos="fade-up">{{ $partnershipSection->title }}</h2>
                    @else
                        <h2 class="section-title text-white" data-aos="fade-up">Tərəfdaşlıq <span class="text-primary">İmkanları</span></h2>
                    @endif
                    @if(isset($partnershipSection) && $partnershipSection->subtitle)
                        <p class="section-subtitle text-light" data-aos="fade-up" data-aos-delay="200">
                            {{ $partnershipSection->subtitle }}
                        </p>
                    @else
                        <p class="section-subtitle text-light" data-aos="fade-up" data-aos-delay="200">
            ...
            <div class="row g-4 align-items-center">
                <div class="col-lg-6" data-aos="fade-right">
                    <div class="partnership-content">
                        ...
                        <div class="partnership-features">
                            @if(isset($partnershipFeatures) && $partnershipFeatures->count() > 0)
                                @foreach($partnershipFeatures as $feature)
                                    <div class="feature-item" data-id="{{ $feature->id }}" data-image="{{ $feature->image_url }}"
                                         data-stat1-number="{{ $feature->stat_number_1 }}" data-stat1-text="{{ $feature->stat_text_1 }}"
                                         data-stat2-number="{{ $feature->stat_number_2 }}" data-stat2-text="{{ $feature->stat_text_2 }}">
                ...
                <div class="col-lg-6" data-aos="fade-left">
                    <div class="partnership-image">
                        <img id="dynamic-partnership-image" src="{{ (isset($partnershipFeatures) && $partnershipFeatures->first() && $partnershipFeatures->first()->image_url) ? $partnershipFeatures->first()->image_url : 'https://images.unsplash.com/photo-1560472354-b33ff0c44a43?ixlib=rb-4.0.3&auto=format&fit=crop&w=1226&q=80' }}" 
                             alt="Tərəfdaşlıq" class="img-fluid" style="border-radius: 8px; object-fit: cover; width: 100%; height: 400px;">
                        
                        <!-- Stats Overlay -->
                        <div class="stats-overlay">
                            <div class="stat-item">
                                <div id="dynamic-stat1-number" class="stat-number">{{ (isset($partnershipFeatures) && $partnershipFeatures->first() && $partnershipFeatures->first()->stat_number_1) ? $partnershipFeatures->first()->stat_number_1 : '3500+' }}</div>
                                <div id="dynamic-stat1-label" class="stat-label">{{ (isset($partnershipFeatures) && $partnershipFeatures->first() && $partnershipFeatures->first()->stat_text_1) ? $partnershipFeatures->first()->stat_text_1 : 'Məmnun Müştəri' }}</div>
                            </div>
                            <div class="stat-item">
                                <div id="dynamic-stat2-number" class="stat-number">{{ (isset($partnershipFeatures) && $partnershipFeatures->first() && $partnershipFeatures->first()->stat_number_2) ? $partnershipFeatures->first()->stat_number_2 : '25' }}</div>
                                <div id="dynamic-stat2-label" class="stat-label">{{ (isset($partnershipFeatures) && $partnershipFeatures->first() && $partnershipFeatures->first()->stat_text_2) ? $partnershipFeatures->first()->stat_text_2 : 'Beynəlxalq Tərəfdaş' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <section class="section-padding text-white" style="background-color: var(--section-bg);">
        <div class="container">
            ...
            <div class="row g-4">
                @if(isset($partnershipTypes) && $partnershipTypes->count() > 0)
                    @foreach($partnershipTypes as $type)
                        <div class="col-lg-4 col-md-6" data-aos="fade-up" data-aos-delay="{{ 300 + ($loop->index * 100) }}">
                            <div class="partnership-card">
                                ...
                                <h4 class="text-white">{{ $type->title }}</h4>
                                <p class="text-light">{{ $type->description }}</p>
            ...
            <div class="row justify-content-center">
                <div class="col-lg-8 text-center" data-aos="fade-up">
                    <div class="cta-content">
                        @if(isset($investorContactSection) && $investorContactSection->title)
                            <h2 class="section-title text-white mb-4">{{ $investorContactSection->title }}</h2>
                        @else
                            <h2 class="section-title text-white mb-4">Bizimlə Tərəfdaşlığa Hazırsınız?</h2>
                        @endif
                        
                        @if(isset($investorContactSection) && $investorContactSection->subtitle)
                            <p class="section-subtitle text-light mb-4">
                                {{ $investorContactSection->subtitle }}
                            </p>
                        @else
                            <p class="section-subtitle text-light mb-4">
                        ...
                        <div class="cta-actions">
                            @if(isset($investorContactSection) && $investorContactSection->button1_text && $investorContactSection->button1_link)
                                <a href="{{ $investorContactSection->button1_link }}" class="btn-primary-custom me-3">
                                    <i class="fas fa-envelope me-2"></i>{{ $investorContactSection->button1_text }}
                                </a>
                            @else
                                <a href="{{ route('front.contact') }}" class="btn-primary-custom me-3">
                                    <i class="fas fa-envelope me-2"></i>Əlaqə Saxlayın
                                </a>
                            @endif
                            
                            @if(isset($investorContactSection) && $investorContactSection->button2_text && $investorContactSection->button2_link)
                                <a href="{{ $investorContactSection->button2_link }}" class="btn-outline-custom">
                                    <i class="fas fa-phone me-2"></i>{{ $investorContactSection->button2_text }}
                                </a>
                            @else
                        ...
                        <div class="contact-info mt-4">
                            @if(isset($investorContactSection) && $investorContactSection->email)
                                <p class="text-light">
                                    <i class="fas fa-envelope text-primary me-2"></i>
                                    {{ $investorContactSection->email }}
                                </p>
                            @else
                                <p class="text-light">
                                    <i class="fas fa-envelope text-primary me-2"></i>
