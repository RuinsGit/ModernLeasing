...
@section('title', 'FAQ Kateqoriyaları - İdarə Paneli')
                        ...
                        @if($categories->count() > 0)
                            <div class="table-responsive">
                                <table class="table align-middle table-nowrap table-check">
                                    ...
                                    <tbody class="sortable" data-url="{{ route('admin.faq-categories.order') }}">
                                        @foreach($categories as $category)
                                            <tr id="order-{{ $category->id }}">
                                                <td>
                                                    <div class="form-check font-size-16">
                                                ...
                                                <td>
                                                    <form action="{{ route('admin.faq-categories.toggle-status', $category->id) }}" method="POST" class="d-inline-block">
                                                        @csrf
                                                        <button type="submit" class="btn btn-sm btn-{{ $category->is_active ? 'success' : 'danger' }}">
                                                ...
                                                <td>
                                                    <div class="d-flex gap-3">
                                                        <a href="{{ route('admin.faq-categories.edit', $category->id) }}" class="text-success">
                                                            <i class="mdi mdi-pencil font-size-18"></i>
                                                        </a>
                                                        <form id="delete-form-{{ $category->id }}" action="{{ route('admin.faq-categories.destroy', $category->id) }}" method="POST" class="d-inline-block">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" class="btn btn-link text-danger p-0" onclick="deleteData({{ $category->id }})">
