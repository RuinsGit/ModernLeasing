...
@section('title', 'FAQ Sualları - İdarə Paneli')
                        ...
                        @if($faqItems->count() > 0)
                            <div class="table-responsive">
                                <table class="table align-middle table-nowrap table-check">
                                    ...
                                    <tbody class="sortable" data-url="{{ route('admin.faq-items.order') }}">
                                        @foreach($faqItems as $item)
                                            <tr id="order-{{ $item->id }}">
                                                <td>
                                                    <div class="form-check font-size-16">
                                                        <input class="form-check-input" type="checkbox" id="faqitemcheck{{ $item->id }}">
                                                        <label class="form-check-label" for="faqitemcheck{{ $item->id }}"></label>
                                                    </div>
                                                </td>
                                                <td>{{ $item->id }}</td>
                                                <td>{{ $item->faqCategory->name ?? 'N/A' }}</td>
                                                <td>{{ Str::limit($item->question, 70) }}</td>
                                                <td>{{ $item->order }}</td>
                                                <td>
                                                    <form action="{{ route('admin.faq-items.toggle-status', $item->id) }}" method="POST" class="d-inline-block">
                                                        @csrf
                                                ...
                                                <td>
                                                    <div class="d-flex gap-3">
                                                        <a href="{{ route('admin.faq-items.show', $item->id) }}" class="text-info">
                                                            <i class="mdi mdi-eye font-size-18"></i>
                                                        </a>
                                                        <a href="{{ route('admin.faq-items.edit', $item->id) }}" class="text-success">
                                                            <i class="mdi mdi-pencil font-size-18"></i>
                                                        </a>
                                                        <form id="delete-form-{{ $item->id }}" action="{{ route('admin.faq-items.destroy', $item->id) }}" method="POST" class="d-inline-block">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" class="btn btn-link text-danger p-0" onclick="deleteData({{ $item->id }})">
