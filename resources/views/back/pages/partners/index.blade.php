...
@section('title', 'Tərəfdaşlar - İdarə Paneli')
                                ...
                                <tbody>
                                    @forelse($partners as $partner)
                                        <tr>
                                            <td>
                                                <div class="form-check font-size-16">
                                                    <input class="form-check-input" type="checkbox" id="partnercheck{{ $partner->id }}">
                                                    <label class="form-check-label" for="partnercheck{{ $partner->id }}"></label>
                                                </div>
                                            </td>
                                            <td>
                                                @if($partner->logo)
                                                    <img src="{{ $partner->logo_url }}" alt="{{ $partner->name }}" class="avatar-md rounded" style="width: 50px; height: 50px; object-fit: contain;">
                                                @else
                                            ...
                                            <td>
                                                <h5 class="font-size-14 mb-1">{{ $partner->name }}</h5>
                                            ...
                                            <td>
                                                <form action="{{ route('admin.partners.toggle-status', $partner->id) }}" method="POST" class="d-inline-block">
                                                    @csrf
                                                    <button type="submit" class="btn btn-sm btn-{{ $partner->is_active ? 'success' : 'danger' }}">
                                                        {{ $partner->is_active ? 'Aktiv' : 'Deaktiv' }}
                                                    </button>
                                                </form>
                                            </td>
                                            <td>
                                                <div class="d-flex gap-3">
                                                    <a href="{{ route('admin.partners.show', $partner->id) }}" class="text-info">
                                                        <i class="mdi mdi-eye font-size-18"></i>
                                                    </a>
                                                    <a href="{{ route('admin.partners.edit', $partner->id) }}" class="text-success">
                                                        <i class="mdi mdi-pencil font-size-18"></i>
                                                    </a>
                                                    <form id="delete-form-{{ $partner->id }}" action="{{ route('admin.partners.destroy', $partner->id) }}" method="POST" class="d-inline-block">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="button" class="btn btn-link text-danger p-0" onclick="deleteData({{ $partner->id }})">
